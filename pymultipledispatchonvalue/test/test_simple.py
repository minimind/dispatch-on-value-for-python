...
class TestBasicMatching(unittest.TestCase):
    ...
    def test_any1(self):
        stream = 1
        pattern = pymultidispatchonvalue.any_a
        (matched, stream_found) = pymultidispatchonvalue.match(stream, pattern)
        assert matched
        assert stream == stream_found
    ...
    def test_any3(self):
        stream = [1, 2]
        pattern = [pymultidispatchonvalue.any_a, 2]
        (matched, stream_found) = pymultidispatchonvalue.match(stream, pattern)
        assert matched
        assert stream == stream_found
    ...
    def test_any6(self):
        stream = [1, 2, 3, [1, 4, 5]]
        pattern = [pymultidispatchonvalue.any_a, 2, 3,
                   [pymultidispatchonvalue.any_a, 4, 5]]
        (matched, stream_found) = pymultidispatchonvalue.match(stream, pattern)
        assert matched
        assert stream == stream_found
    ...
    def test_any8(self):
        ...
        pattern = {'a': 1, 'b': pymultidispatchonvalue.any_b,
                   'c': {'e': 3, 'd': pymultidispatchonvalue.any_b}}
        (matched, stream_found) = pymultidispatchonvalue.match(stream, pattern)
        assert matched
        assert stream == stream_found
